...
export default function NovelSettingsPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const n = novelData.novel;
        setNovel(n);
        setTitle(n.title);
        setSynopsis(n.synopsis);
        setGenreId(n.genre?.id || n.genreId);
        setStatus(n.status);
        setIsAdult(n.isAdult);
        setTags(n.tags?.map((t: NovelTag) => t.name) || []);
        setExistingCover(n.coverImage);
  ...
  // Check ownership
  useEffect(() => {
    if (novel && session?.user) {
      if (novel.authorId !== session.user.id && userRole !== "ADMIN") {
        router.push("/write");
      }
    }
  }, [novel, session, userRole, router]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      ...
      setExistingCover(data.novel.coverImage);
      setCoverFile(null);
      setCoverPreview(null);
      setRemoveCover(false);
      if (fileInputRef.current) {
  ...
  const displayCover = coverPreview || (removeCover ? null : existingCover);
      ...
      <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            แก้ไขข้อมูลของ &ldquo;{novel.title}&rdquo;
          </p>
        </div>
            ...
            <div className="flex flex-col sm:flex-row gap-6">
              ...
              <div className="flex-1">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  {existingCover && !removeCover && !coverPreview && (
                    <Button
                      type="button"
                      variant="ghost"
                      onClick={handleRemoveCover}
                ...
                <button
                  ...
                  className={cn(
                    ...
                    status === opt.value
                      ...
                      : "border-border bg-white text-brand-black/70 hover:border-rosegold/50"
                  )}
                >
                  {opt.label}
                </button>
